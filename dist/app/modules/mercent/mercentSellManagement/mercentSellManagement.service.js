...
//   return sell;
// };
const checkout = (merchantId_1, digitalCardCode_1, totalBill_1, ...args_1) => __awaiter(void 0, [merchantId_1, digitalCardCode_1, totalBill_1, ...args_1], void 0, function* (merchantId, digitalCardCode, totalBill, promotionIds = [], pointRedeemed = 0) {
    console.log("=================================================");
    ...
    // ===============================
    const cardUser = yield user_model_1.User.findById(digitalCard.userId);
    if (!cardUser) {
    ...
    let totalGrossValue = 0;
    for (const promoId of promotionIds) {
        const promoInCard = digitalCard.promotions.find(p => { var _a; return ((_a = p.promotionId) === null || _a === void 0 ? void 0 : _a.toString()) === promoId; });
        if (!promoInCard) {
        ...
        //   throw new Error(`Promotion ${promoId} already used`);
        // }
        const promotion = yield promotionMercent_model_1.Promotion.findById(promoId);
        if (!promotion) {
    ...
    //  Tier upergread 
    // ==============================
    const nextTier = yield tier_model_1.Tier.findOne({
        admin: merchantId,
        isActive: true,
        pointsThreshold: { $lte: digitalCard.lifeTimeEarnPoints || 0 },
    }).sort({ pointsThreshold: -1 });
    console.log("✅ User Tier:", nextTier ? nextTier.name : "No tier");
    const upcomingTier = yield tier_model_1.Tier.findOne({
        admin: merchantId,
        isActive: true,
    ...
    if (digitalCard.updatedAt < sixMonthsAgo && pointRedeemed > 0) {
        throw new Error("Your points are blocked due to inactivity in the last 6 months.");
    }
    ...
    const userTier = yield tier_model_1.Tier.findOne({
        admin: merchantId,
        isActive: true,
        pointsThreshold: { $lte: digitalCard.lifeTimeEarnPoints || 0 },
    }).sort({ pointsThreshold: -1 });
    const accumulationPercentage = (userTier === null || userTier === void 0 ? void 0 : userTier.accumulationRule) || 0;
    ...
    const totalPointsAfterTxn = (digitalCard.lifeTimeEarnPoints || 0) + pointsEarned;
    // 🔥 NEW: get all tiers
    const allTiers = yield tier_model_1.Tier.find({
        admin: merchantId,
        isActive: true,
    }).sort({ pointsThreshold: 1 });
    ...
    // 🧾 Create Sell Record
    // ===============================
    const sell = yield mercentSellManagement_model_1.Sell.create({
        merchantId,
        userId: digitalCard.userId,
        digitalCardId: digitalCard._id,
        digitalCardCode,
...
// -----------------------------
const requestApproval = (_a) => __awaiter(void 0, [_a], void 0, function* ({ merchantId, digitalCardCode, // DG-xxxx or PC-xxxx
promotionId = null, // string | string[]
totalBill = 0, pointRedeemed = 0, }) {
    const POINT_EARN_RATE = 10; // 10 taka = 1 point
    const POINT_REDEEM_RATE = 10; // 1 point = 10 taka
    // -----------------------------
    // 🔹 Check if merchant has at least one active tier
    // -----------------------------
    const activeTierExists = yield tier_model_1.Tier.exists({
        admin: merchantId,
        isActive: true,
    });
    if (!activeTierExists) {
        throw new Error("Merchant has no active tier. Please create a tier before processing sales.");
    }
    ...
    if (digitalCardCode === null || digitalCardCode === void 0 ? void 0 : digitalCardCode.startsWith("PC-")) {
        ...
        const promo = digitalCard.promotions.find((p) => p.promoCode === digitalCardCode);
        if (!promo)
    ...
    else if (digitalCardCode === null || digitalCardCode === void 0 ? void 0 : digitalCardCode.startsWith("DG-")) {
        digitalCard = yield digitalCard_model_1.DigitalCard.findOne({
            merchantId,
            cardCode: digitalCardCode,
        });
        if (!digitalCard)
            throw new Error("Digital card not found by card code");
    }
    else {
    ...
    if (totalBill < totalGrossValue) {
        throw new Error(`Total bill (${totalBill}) is less than total promotions gross value (${totalGrossValue})`);
    }
    // ===============================
    // 💰 Bill Calculation
    // ===============================
    ...
    //   );
    // }
    const finalBill = parseFloat((discountedBills - pointDiscount).toFixed(4));
    if (finalBill < 0) {
        throw new Error("Final bill cannot be negative");
    }
...
const getPendingRequests = (userId) => __awaiter(void 0, void 0, void 0, function* () {
    const cards = yield digitalCard_model_1.DigitalCard.find({
...
// };
const approvePromotion = (digitalCardCode, promotionIds, userId, sellId) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("=================================================");
    ...
    // 🔍 Find pending sell
    // -------------------------------
    const sell = yield mercentSellManagement_model_1.Sell.findOne({
    ...
    for (const promotionId of promotionIds) {
        const promo = digitalCard.promotions.find(p => { var _a; return ((_a = p.promotionId) === null || _a === void 0 ? void 0 : _a.toString()) === promotionId; });
    ...
    digitalCard.availablePoints = parseFloat(((digitalCard.availablePoints || 0) - redeemed + earned).toFixed(4));
    digitalCard.lifeTimeEarnPoints = parseFloat(((digitalCard.lifeTimeEarnPoints || 0) + earned).toFixed(4));
...
const approvePromotionReject = (digitalCardCode, promotionIds, userId, sellId) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("=================================================");
    ...
    if (sellId)
        sellQuery._id = sellId;
    const sell = yield mercentSellManagement_model_1.Sell.findOne(sellQuery);
    if (!sell)
        throw new Error("Pending checkout not found");
    console.log("📌 Pending Sell found:", sell._id.toString());
    console.log("🔹 Sell Total Bill:", sell.totalBill);
...
// Service
const getPointsHistory = (digitalCardId_1, ...args_1) => __awaiter(void 0, [digitalCardId_1, ...args_1], void 0, function* (digitalCardId, type = "all") {
    var _a, _b, _c;
    if (!mongoose_1.Types.ObjectId.isValid(digitalCardId)) {
        throw new Error("Invalid digitalCardId");
    }
    const typeSanitized = type.trim().toLowerCase();
    const query = {
        digitalCardId: new mongoose_1.Types.ObjectId(digitalCardId),
    };
    if (typeSanitized === "earn")
        query.pointsEarned = { $gt: 0 };
    if (typeSanitized === "use")
        query.pointRedeemed = { $gt: 0 };
    const history = yield mercentSellManagement_model_1.Sell.find(query)
        .sort({ createdAt: -1 })
    ...
    for (const tx of history) {
        ...
        const merchantName = (merchant === null || merchant === void 0 ? void 0 : merchant.businessName) || (merchant === null || merchant === void 0 ? void 0 : merchant.shopName) || (merchant === null || merchant === void 0 ? void 0 : merchant.firstName) || "";
        const ratingDoc = yield rating_model_1.Rating.findOne({
            userId: tx.userId,
            merchantId: merchant === null || merchant === void 0 ? void 0 : merchant._id,
            promotionId: (_a = tx.promotionIds) === null || _a === void 0 ? void 0 : _a[0], // আমরা প্রথম promotion দিয়ে rating check করছি
        ...
        for (const promoId of promotions) {
            // ✅ Earn Points
            if ((typeSanitized === "all" || typeSanitized === "earn") && ((_b = tx.pointsEarned) !== null && _b !== void 0 ? _b : 0) > 0) {
                result.push({
                    id: tx._id,
                    digitalCardId: tx.digitalCardId,
                    isEarn: true,
                    ...
                    merchantProfile: merchant === null || merchant === void 0 ? void 0 : merchant.profile,
                    merchantId: merchant === null || merchant === void 0 ? void 0 : merchant._id,
                    rating: ratingValue,
                });
            }
            // ✅ Used Points
            if ((typeSanitized === "all" || typeSanitized === "use") && ((_c = tx.pointRedeemed) !== null && _c !== void 0 ? _c : 0) > 0) {
                result.push({
                    id: tx._id,
                    digitalCardId: tx.digitalCardId,
                    isEarn: false,
...
const getUserFullTransactions = (userId_1, ...args_1) => __awaiter(void 0, [userId_1, ...args_1], void 0, function* (userId, type = "all", page = 1, limit = 20) {
    ...
    const [total, transactions] = yield Promise.all([
        mercentSellManagement_model_1.Sell.countDocuments(query),
        mercentSellManagement_model_1.Sell.find(query)
